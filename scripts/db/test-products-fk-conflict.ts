...
//      ter 0 testes pendentes. Desde o WP-R6 o gate do teste é
//      `dbPrecondition()` (`src/test/helpers/db-precondition.ts`): **tudo ou
//      nada, e falha alta** — nenhuma URL definida ⇒ o bloco pula (e o pending
//      abaixo reprova); qualquer uma definida ⇒ o par
//      `DATABASE_ADMIN_URL` + `DATABASE_URL` é exigido em loopback, e
//      `DATABASE_URL_UNPOOLED` é validada se definida (é o kill-switch contra
//      credencial de produção herdada). A versão anterior devolvia `true` para
//      valor ausente e por isso o bloco **executava** com uma URL faltando;
//      aquele comportamento foi substituído de propósito. Medido em
//      2026-09-21: par em loopback ⇒ `13 total / 13 passed / 0 pending`;
//      `DATABASE_URL_UNPOOLED` remota ⇒ reprova alto nomeando a chave;
 ...
 *
 * Subiu de 13 para 15 em 2026-09-20 com o `F-D2-depth-pin`, que acrescentou os
 * dois casos de fronteira da cadeia de `cause`. O piso tem de acompanhar a
 * contagem do arquivo: um piso que fica para trás permite apagar em silêncio
 * exactamente a prova que ele existe para proteger.
 */
  ...
  try {
    ...
    console.log(
      `prova de banco (${testFile}) contra 127.0.0.1: ${summary.numPassedTests} passed ` +
        `(${summary.numTotalTests}), ${summary.numPendingTests} skipped — admin=${new URL(adminUrl).host}`,
    );
  } finally {
